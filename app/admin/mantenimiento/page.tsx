    ...
    <main className="px-6 py-10">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {venues.map((v) => (
          <Link
            key={v.id}
            href={`/admin/mantenimiento?venueId=${v.id}&date=${dateIso}`}
      ...
      <div className="mt-4 flex items-center justify-between">
        <Link
          href={`/admin/mantenimiento?venueId=${venue.id}&date=${prevDateIso}`}
        ...
        <Link
          href={`/admin/mantenimiento?venueId=${venue.id}&date=${nextDateIso}`}
          className="text-sm text-gray-500 hover:underline"
        >
          Día siguiente →
        </Link>
